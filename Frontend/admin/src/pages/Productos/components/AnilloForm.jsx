...
export default function AnilloForm({ type = "create", initialData = {}, onClose }) {
  const [formData, setFormData] = useState({
    ...
    tipo_anillo: initialData.tipo_anillo || "compromiso",
    photo: null,
    photo2: null,
    photo3: null,
    ...initialData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        if (key.includes("photo") && formData[key]) {
      ...
      const response = await fetch(
        type === "create" ? "/api/anillos/" : `/api/anillos/${formData.id}/`,
        {
      ...
      if (!response.ok) {
        throw new Error("Error al guardar el anillo");
      }

      alert(
        type === "create"
          ? "Anillo creado con éxito"
          : "Anillo actualizado con éxito"
      );
      ...
      alert("Error al guardar el anillo: " + error.message);
    }
  };
    ...
    <form
      ...
      <div className="flex justify-end space-x-3">
        ...
        <button
          ...
          {type === "create" ? "Crear Anillo" : "Actualizar Anillo"}
        </button>
      </div>
    </form>
  );
