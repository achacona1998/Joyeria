...
export default function AreteForm({ type = "create", initialData = {}, onClose }) {
  const [formData, setFormData] = useState({
    ...
    unidades: initialData.unidades || "pareja",
    photo: null,
    photo2: null,
    photo3: null,
    ...initialData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        type === "create" ? "/api/aretes/" : `/api/aretes/${formData.id}/`,
        {
      ...
      if (!response.ok) {
        throw new Error("Error al guardar el arete");
      }

      alert(
        type === "create"
          ? "Arete creado con éxito"
          : "Arete actualizado con éxito"
      );
      ...
      alert("Error al guardar el arete: " + error.message);
    }
  };
    ...
    <form
      ...
      <div className="flex justify-end space-x-3">
        ...
        <button
          ...
          {type === "create" ? "Crear Arete" : "Actualizar Arete"}
        </button>
      </div>
    </form>
  );
